...
transaction(receiverAddress: Address, privatePath: Path) {
    ...
    let receveiverAdminProxyRef: &MFLAdmin.AdminProxy{MFLAdmin.AdminProxyPublic}
    let squadAdminClaimCapability: Capability<&MFLClub.SquadAdmin{MFLClub.SquadAdminClaim}>
    ...
    prepare(acct: AuthAccount) {
        ...
        self.receveiverAdminProxyRef = receiverAccount.getCapability<&MFLAdmin.AdminProxy{MFLAdmin.AdminProxyPublic}>(MFLAdmin.AdminProxyPublicPath).borrow() ?? panic("Could not borrow AdminProxyPublic ref")
        let privateCapabilityPath = privatePath as? PrivatePath
        self.squadAdminClaimCapability = acct.link<&MFLClub.SquadAdmin{MFLClub.SquadAdminClaim}>(privateCapabilityPath!, target: MFLClub.SquadAdminStoragePath) ?? panic("path already exists")
    }
