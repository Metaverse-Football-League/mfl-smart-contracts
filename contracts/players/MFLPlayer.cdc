...
contract MFLPlayer: NonFungibleToken {
	...
	fun resolveViewFromData(_ view: Type, playerData: PlayerData): AnyStruct? {
		var playerImageUrl = MFLAdmin.imageHostUrl()
			.concat("/players/").concat(playerData.id.toString())
			.concat("/card.png?co=").concat((playerData.metadata["overall"] as! UInt32?)!.toString())
		if let retired = playerData.metadata["retired"] as! String? {
			playerImageUrl = playerImageUrl.concat("R")
		}
			...
			case Type<MetadataViews.NFTCollectionDisplay>():
				return MetadataViews.NFTCollectionDisplay(
					...
					squareImage: MetadataViews.Media(
						file: MetadataViews.HTTPFile(url: "https://app.playmfl.com/img/collections/players/thumbnail.png"),
						mediaType: "image/png"
					),
					bannerImage: MetadataViews.Media(
						file: MetadataViews.HTTPFile(url: "https://app.playmfl.com/img/collections/players/banner.png"),
						mediaType: "image/png"
					),
					socials: {
